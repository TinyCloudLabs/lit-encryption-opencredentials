// Pure JavaScript string version to avoid TypeScript compilation artifacts
export const litActionCodeWithES256K = `
(async () => {
  try {
    ...
    function verifyJWTWithEdDSA(jwt, issuerDID) {
      return new Promise(function(resolve, reject) {
        try {
          ...
          resolveDIDWeb(issuerDID).then(function(didDocument) {
            ...
            var dataToVerify = new TextEncoder().encode(headerB64 + '.' + payloadB64);
            
            // 8. Decode signature from base64url
            var signature = base64UrlDecode(signatureB64);
            
            // 9. Verify signature
            return crypto.subtle.verify('Ed25519', publicKey, signature, dataToVerify);
    ...
    var verificationResult = await verifyJWTWithEdDSA(credentialJWT, credentialRequirements.issuer);
    ...
    Lit.Actions.setResponse({ 
      response: {
        ...
        verifiedCredential: {
          ...
          githubHandle: verificationResult.payload.vc.evidence && verificationResult.payload.vc.evidence.handle,
    ...
    Lit.Actions.setResponse({ 
      response: {
        success: false,
        error: e.message,
        errorType: e.message.indexOf('ES256K') !== -1 ? 'user_jwt_verification' : 
                  e.message.indexOf('GitHub') !== -1 ? 'github_credential_verification' :
                  e.message.indexOf('decrypt') !== -1 ? 'decryption_error' : 'general_error'
      }
    });
  }
})();`;